...
@objcMembers
public final class PIPCustomLayout: NSObject, Codable, Sendable {
    ...
    static func save(layout: PIPCustomLayout) {
    ...
    static func restore(_ placeholder: PIPCustomLayout? = nil) -> PIPCustomLayout {
        guard let data = UserDefaults.standard.data(forKey: userDefaultsKey) else {
            return placeholder ?? .init(bottom: 0.0, trailing: 0.0)
        }
        do {
            return try JSONDecoder().decode(PIPCustomLayout.self, from: data)
        } catch _ {
