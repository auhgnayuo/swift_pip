...
extension PIP {
    ...
    @objcMembers
    @MainActor
    open class CustomController: NSObject {
        ...
        open var contentSource: ContentSource? {
            didSet {
        ...
        open weak var delegate: CustomControllerDelegate?
        ...
        open var placeholderLayout: PIP.CustomController.Layout = .init(bottom: 0.45, trailing: 0.0)
        ...
        public init(contentSource: ContentSource? = nil) {
        ...
        open func startPictureInPicture() {
            ...
            Window.singleton.containerViewController.contentSource = contentSource
        }
        ...
        open func stopPictureInPicture() {
            ...
            if Window.instance?.containerViewController.contentSource?.controller == self {
                Window.instance?.containerViewController.contentSource = nil
            }
        }
        ...
        open func expand() {
            if Window.instance?.containerViewController.contentSource?.controller == self {
                Window.instance?.containerViewController.expand()
            }
        }
        ...
        open func collapse() {
            if Window.instance?.containerViewController.contentSource?.controller == self {
                Window.instance?.containerViewController.collapse()
            }
        }
        ...
        private var strongDelegate: CustomControllerDelegate?
...
extension PIP.CustomController {
    ...
    func restoreUserInterfaceStopWithCompletionHandler(_ completionHandler: @escaping (Bool) -> Void) {
        if delegate?.responds(to: #selector(PIP.CustomControllerDelegate.customPictureInPictureController(_:restoreUserInterfaceForPictureInPictureStopWithCompletionHandler:))) == true {
